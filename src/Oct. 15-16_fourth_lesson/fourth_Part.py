...
# we implement prediction using a double for loop
num_samples = x.shape[0]
num_centroids = centroids.shape[0]
dist = np.zeros(shape=(num_samples, num_centroids))
for j in range(num_centroids):
    for i in range(num_samples):
        u = x[i, :]
        v = centroids[j, :]
        dist[i, j] = ((u - v) * (u - v)).sum()  # d(u,v)  #np.dot(u-v,u-v)
...
num_samples = x.shape[0]
num_centroids = centroids.shape[0]
dist1 = np.zeros(shape=(num_samples, num_centroids))
for j in range(num_centroids):
    v = centroids[j, :]
    dist1[:, j] = ((x - v) * (x - v)).sum(axis=1)
end_time = timeit.default_timer()
print("time elapsed: ", end_time - start_time)
...
start_time = timeit.default_timer()
dist2 = euclidean_distances(x, centroids, squared=True)
end_time = timeit.default_timer()
print("time elapsed: ", end_time - start_time)
